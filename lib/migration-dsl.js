...
MigrationDSL.prototype = {
  ...
  createTable: function(collectionName, options, cb){
    ...
    for (var k in options) {
      ...
      // SQLite might already have been made a primary key - if so, don't do it again.
      if (options[k].primary && !/(^|\W)PRIMARY KEY(\W|$)/.test(col.value)) {
        primary.push(k);
      }
...
Resumer.prototype.currentMigration = function currentMigration(positioner, cb){
  var sql = "SELECT migration FROM " + this.task.tableName + " ORDER BY created_at DESC LIMIT 1;";
...
Recorder.prototype.record = function record(item, cb) {
  ...
  this.execute(
    "INSERT INTO " + this.task.tableName + "(migration, direction, created_at) VALUES(?, ?, ?)",
...
Resumer.prototype.ensureMigrationsTable = function(cb) {
  ...
  dsl.createTable(this.task.tableName, orm2Migrations, recallPositionFallback);
}

//----- patch around node-migrate libs
module.exports = function(dbDriver, task){
  var dsl = new MigrationDSL(dbDriver);
  var recorder = new Recorder(dbDriver, dsl, task);
